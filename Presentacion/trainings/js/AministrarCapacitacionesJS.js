...
function GetCapacitaciones() {
    ejecutarajax("AdministrarCapacitaciones.aspx/GetCapacitaciones", { "info": $("#txtSearch").val()}, CargarCapacitaciones)
}
...
$("#btnIniciar").click(function (e) {
    let idAgenda = $(e.target).attr("data-idagenda")
    ejecutarajax(
        "AdministrarCapacitaciones.aspx/IniciarCapacitacion",
        { "idAgenda": idAgenda },
        function (msg) {
            window.location.href = "EjecutarCapa.aspx?idAgenda=" + idAgenda;
        }
...
function GetAcataAsistencia(idCapacitacion, Estado) {
    if (Estado != 3) {
        error("Capacitación no terminada", "Mientras la capacitación se encuentre no iniciado o en proceso no se puede ver el acta de asistencia");
    }
    else {
...
function ConfiurarAgenda(idAgenda, estado) {
    ...
    if(estado == 1)
        ...
        error("Capacitación iniciada/terminada","Mientras la capacitación esté iniciada/terminada no se podra editar")
}
...
function GetAgenda(index) {
    ...
    ejecutarajax("AdministrarCapacitaciones.aspx/GetAgendasByIdCapacitacion", {"idCapacitacion": idCapacitacion}, CargarAgendas)
}
