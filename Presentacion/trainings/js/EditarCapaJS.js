...
//metodo que realiza las solicitudes al servidor por ajax
...
$(document).on("click", ".btn-delete-arch", e => {
    ...
    ejecutarajax(
        "EditarCapa.aspx/deleteArchivoSubido",
        { 'index': index },
        buscarArchivosSubidos
    )
});
...
function CargarAgenda(msg) {
    ...
    ejecutarajax("EditarCapa.aspx/GetArchivosSubidos", {}, buscarArchivosSubidos);
...
function UpdateAgenda() {
    ...
    ejecutarajax("EditarCapa.aspx/UpdateAgtenda", { "agenda": Agenda, "subtemas": GetSubtemas() }, () => window.location.href = "AdministrarCapacitaciones.aspx" );
}

$("#btnActulizarAgenda").click(function (e) {
    e.preventDefault();
    UpdateAgenda();
});
...
function GetAgenda() {
    ejecutarajax("EditarCapa.aspx/GetAgenda", {"idAgenda": idAgenda}, CargarAgenda)
}
