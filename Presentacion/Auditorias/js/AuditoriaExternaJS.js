...
$("#btnAuditor").click(function (e) {
    e.preventDefault();
    let subTema = $("#txtAuditores").val()
...
$("#btnHallazgo").click(function (e) {
    ...
    let responsable = $("#ContentPlaceHolder_ddlResponsable").val();
    let textResponsable = $("#ContentPlaceHolder_ddlResponsable option:selected").text();
...
function ValidarFormulario() {
    ...
    if ($("#ContentPlaceHolder_ddlEnteAuditor").val() == "-1") {
        error("No se seleccionado el ente auditor", "Por favor seleccione el ente auditor de la lista desplegable")
        return false;
    }
    if (GetAuditores().length == 0) {
        error("No se ha ingresado ningun auditor", "Por favor ingrese al menos un auditor")
        return false;
    }
...
function BorrarFormulario() {
    $("#txtFecha").val("");
    $("#ContentPlaceHolder_ddlEnteAuditor").val("-1");
    $("#txtObjetivo").val("");
    $("#txtAlcance").val("");
...
$("#ContentPlaceHolder_btnGuardar").on("click", function (e) {
    ...
    let auditoria = {
        ...
        'StrEnteAuditor': $("#ContentPlaceHolder_ddlEnteAuditor").val(),
        'StrAuditores': JSON.stringify(GetAuditores()),
    ...
    ejecutarajax(
        "AuditoriaExterna.aspx/SetAuditoriaExterna",
        {
            "auditoria": auditoria,
...
function GetAuditores() {
    let auditores = [];
    document.querySelectorAll("#lstAuditores .item-list").forEach(itemAuditor => {
        auditores.push(itemAuditor.innerText);
    })
    return auditores;
}
...
$("#ContentPlaceHolder_fuArchivo").on("change", async function (e) {
    ...
    await ejecutarajax(
        "AuditoriaExterna.aspx/cargarArchivo",
        { 'archivo': archivo },
...
$(document).on("click", ".btn-delete-arch", e => {
    ...
    ejecutarajax(
        "AuditoriaExterna.aspx/deleteArchivoSubido",
        { 'index': index },
        buscarArchivosSubidos
    )
});